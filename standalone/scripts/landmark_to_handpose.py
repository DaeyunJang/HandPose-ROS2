...
class LandmarkToHandPose:
    ...
    def __init__(self, landmarks: Optional[np.ndarray] = None,
                 flip_x: bool = True, hand_label: str = "left"):
    ...
    def _build_wrist_frame(self, label: str):
        ...
        R_in2w = np.stack([x_hat, y_hat, z_hat], axis=1)
        t_in2w = p0.ravel()
        T_in2w = make_homogeneous(R_in2w, t_in2w)
        T_w2in = make_inv_homogeneous(T_in2w)

        lm_h = np.hstack([lm, np.ones((lm.shape[0], 1))])
        lm_w = (T_w2in @ lm_h.T).T[:, :3]
        return T_in2w, T_w2in, lm_w
    ...
    def _compute_wrist_to_joint_transforms(
        ...
        v3d = (p_next - p_joint).ravel()
        v_xy = v3d * np.array([1,1,0])

        psi = angle_between_vectors(x_unit, v_xy, z_unit, degrees=False)
        Rz  = R.from_euler('z', psi, degrees=False)

        y_axis_rot = Rz.apply(y_unit)
        phi = angle_between_vectors(v_xy, v3d, y_axis_rot, degrees=False)
        ...
        return make_homogeneous(R_w2j.as_matrix(), p_joint.ravel())
    ...
    def get_frame(self, reference: Tuple[str, str], target: Tuple[str, str]) -> np.ndarray:
        ...
        def world_of(node: Tuple[str,str]) -> np.ndarray:
            return np.eye(4) if node[0]=='wrist' else self.frames.T_wrist2joint[(node[0], node[1])]
        T_w2ref = world_of(reference); T_w2tgt = world_of(target)
        return make_inv_homogeneous(T_w2ref) @ T_w2tgt

    def compute(self, label: str) -> HandFrames:
        T_in2w, T_w2in, lm_wrist = self._build_wrist_frame(label=label)
        T_wrist2joint: Dict[Tuple[str,str], np.ndarray] = {}
        ...
        self.frames = HandFrames(
            T_input2wrist=T_in2w, T_wrist2input=T_w2in,
    ...
    def plot(self, show_labels: bool = True, frame_scale: float = 0.02, joint_size: int = 28):
        assert self.frames is not None, "call compute() first."
        lm_w = self.frames.landmarks_wrist
        ...
        wi = self.WRIST_INDEX
        ax.scatter(lm_w[wi,0], lm_w[wi,1], lm_w[wi,2], s=55, c='k')
        ...
        for finger, idx_list in finger_joint_indices.items():
            ...
            mcp_idx = idx_list[0]
            ax.plot([lm_w[wi,0], lm_w[mcp_idx,0]],
                    [lm_w[wi,1], lm_w[mcp_idx,1]],
                    [lm_w[wi,2], lm_w[mcp_idx,2]], '--', color=color, linewidth=1.6)

        if show_labels:
            for i in range(lm_w.shape[0]):
                ax.text(lm_w[i,0], lm_w[i,1], lm_w[i,2], f" {i}", fontsize=8, color='k')
        ...
        mins = lm_w.min(axis=0); maxs = lm_w.max(axis=0)
...
if __name__ == "__main__":
    # 데모: 엑셀 로드 사용 시
    solver = LandmarkToHandPose.from_excel("handpose_lefthand_data2.xlsx", sheet_name=0, header=None, flip_x=True)
